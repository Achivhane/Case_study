...
export class PaymentComponent implements OnInit {
  ...
  constructor(
    private service: PayemntServiceService,
    private route: ActivatedRoute,
    private formBuilder: FormBuilder,
    private router: Router,
    private get: PayemntServiceService
  ) {}
  ...
  ngOnInit(): void {
    ...
    this.service.getPaymentPending(this.id).subscribe(
      (data) => {
  ...
  public paymentId(id: any) {
    console.log('id ' + id);
    this.get.postPaymentDetails(id).subscribe(
      (data) => {
        console.log(data);
